...
# Use (5) as implementation instead of PR

class FixedPointSolver(nn.Module):
    def __init__(self, lin_module, nonlin_module, alpha=1.0, fp_tol=1e-5, max_iter=300, update_alpha=False, alpha_factor=1, verbose=True, record=True, **kwargs):
        super().__init__()
        ...
        # self.stats.set_options(alpha=alpha,fp_tol=fp_tol,max_iter=max_iter,verbose=True,**kwargs)
        self.record = record
    ...
    def forward(self,x):
        ...
        with torch.no_grad():
            ...
            z = torch.zeros(self.lin_module.z_shape(), dtype=x.dtype, device=x.device)
            u = torch.zeros(self.lin_module.z_shape(), dtype=x.dtype, device=x.device)
        ...
        if self.record:
            ...
            # print('Top 5 Eigs:',*sorted_eigs[-5:],sep=', ')
...
class PeacemanRachford(FixedPointSolver):
    def __init__(self, lin_module, nonlin_module, alpha=1.0, fp_tol=1e-5, max_iter=50, verbose=False, **kwargs):
        super().__init__(lin_module,nonlin_module,alpha,fp_tol,max_iter,verbose,**kwargs)
        pass
...
class AcceleratedSolver(FixedPointSolver):
    ...
    def forward(self,x):
        ...
        with torch.no_grad():
            ...
            Z = torch.zeros(self.m, numel, dtype=x.dtype, device=x.device)
            F = torch.zeros(self.m, numel, dtype=x.dtype, device=x.device)
            bias = self.lin_module.bias(x)
