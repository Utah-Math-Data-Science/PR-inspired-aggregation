...
import torch
from torch.nn import Linear, Module, ModuleList, Parameter, ReLU
import torch.nn.functional as F
import torch.optim as optim
from torch_geometric.seed import seed_everything
from torch_geometric.utils import to_dense_adj

from _conv import BernConv
...
class Model(Module):
    def __init__(self, in_channels, hidden_channels, hidden_layers, out_channels, dropout, K):
        ...
        for i in range(hidden_layers):
            self.conv.append(BernConv(hidden_channels, hidden_channels, K))
        self.coe = Parameter(torch.Tensor(K+1))

        self.reset_parameters()

    def reset_parameters(self):
        self.coe.data.fill_(1.0)

    def forward(self, x, edge_index, edge_weight):
        x = self.enc(x)
        _layers = [x]
        for conv in self.conv:
            x = self.act( conv(x, edge_index, self.coe) )
            _layers.append(x)
        x = self.dec(x)
        return x
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/bernet/", config_name="synth_chains.yaml")
def run_chains(cfg):
    ...
    wandb.init(entity='utah-math-data-science',
                ...
                mode=mode,
                name='bernet-multi-class-100-9'+str(cfg.data['chain_len']),
                dir='/root/workspace/out/',
                tags=['synth-chains', 'bernet', str(cfg.data['chain_len'])],
