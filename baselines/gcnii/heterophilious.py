...
class Model(Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        for conv in self.conv:
            inner = conv(x, x_0=_layers[0], edge_index=edge_index, edge_weight=edge_weight)
            inner = F.dropout(x, p=self.dropout, training=self.training)
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/gcnii/", config_name="heterophilious")
def run_heterophilious(cfg):
    ...
    wandb.init(
        ...
        mode=mode,
        name='gcnii-'+cfg.load['dataset'],
        project='pr-inspired-aggregation',
        tags=['gcnii', cfg.load['dataset']],
