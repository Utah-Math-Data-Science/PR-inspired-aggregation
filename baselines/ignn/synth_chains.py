...
class Model(Module):
    def __init__(self,
        ...
        self.ig1 = ImplicitLayer(in_channels, hidden_channels, num_nodes, kappa)
    ...
    def forward(self, x, edge_index, edge_weight, adj):
        x = self.ig1(self.X_0, adj, x.T, F.relu, A_orig=adj, fw_mitr=self.max_iter, bw_mitr=self.max_iter).T
...
def train(cfg, data, model, optimizer, adj):
    model.train()
    optimizer.zero_grad()
...
def validate(cfg, data, model, adj):
    model.eval()
    output = model(data.x, data.edge_index, data.edge_weight, adj)
    output = F.log_softmax(output, dim=1)
...
def run_training(cfg, data, model):
    ...
    for epoch in range(args['epochs']):
        ...
        start = time.time()
        train_loss, train_acc = train(cfg, data, model, optimizer, adj)
        end = time.time()
        postrain_mem = memory_usage(cfg.setup['device'],key='Postrain')
        val_loss, val_acc = validate(cfg, data, model, adj)
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/ignn/", config_name="synth_chains.yaml")
def run_chains(cfg):
    ...
    wandb.init(entity='utah-math-data-science',
                ...
                mode=mode,
                name='ignn-multi-class'+str(cfg.data['chain_len']),
    ...
    model.to(cfg.setup['device'])
    adj = to_dense_adj(edge_index=data.edge_index, edge_attr=data.edge_weight)[0]
    test_loss, test_acc = test(cfg, data, model, adj)
