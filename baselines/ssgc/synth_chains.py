...
class Model(Module):
    def __init__(self, in_channels, hidden_channels, hidden_layers, out_channels, dropout, alpha, K):
        ...
        for i in range(hidden_layers):
            self.conv.append(SSGConv(in_channels=hidden_channels, out_channels=hidden_channels, K=K, alpha=alpha, normalize=False))
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/ssgc/", config_name="synth_chains.yaml")
def run_chains(cfg):
    ...
    wandb.init(entity='utah-math-data-science',
                ...
                tags=['synth-chains', 'ssgc', str(cfg.data['chain_len'])],
