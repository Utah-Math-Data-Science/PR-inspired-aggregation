...
import torch
from torch.nn import GELU, LayerNorm, Linear, Module, ModuleList, ReLU, Sequential, Tanh
...
class Model(Module):
    def __init__(self,
        ...
        lin_module = CayleyLinear(hidden_channels, hidden_channels, num_nodes, invMethod='direct', adj=adj, device='cuda')
        nonlin_module = ReLU()
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/mignn/", config_name="heterophilious")
def run_heterophilious(cfg):
    ...
    wandb.init(
        ...
        mode=mode,
        name='mignn-'+cfg.load['dataset'],
        project='pr-inspired-aggregation',
        tags=['mignn', cfg.load['dataset']],
