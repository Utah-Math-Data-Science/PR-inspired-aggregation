...
class Model(Module):
    def __init__(self, in_channels, hidden_channels, hidden_layers, out_channels, dropout, K):
        ...
        for i in range(hidden_layers):
            self.conv.append(SGConv(in_channels=hidden_channels, out_channels=hidden_channels, K=K, cached=True))
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/sgc/", config_name="synth_chains.yaml")
def run_chains(cfg):
    ...
    wandb.init(entity='utah-math-data-science',
                ...
                mode=mode,
                name='ignn-multi-class-100-9'+str(cfg.data['chain_len']),
                dir='/root/workspace/out/',
                tags=['synth-chains', 'sgc', str(cfg.data['chain_len'])],
