...
class Model(Module):
    def __init__(self, in_channels, hidden_channels, hidden_layers, out_channels, dropout, K):
        ...
        for i in range(hidden_layers):
            self.conv.append(ChebConv(hidden_channels, hidden_channels, K))
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/baselines/chebnet/", config_name="synth_chains.yaml")
def run_chains(cfg):
    ...
    wandb.init(entity='utah-math-data-science',
                ...
                mode=mode,
                name='chebnet-multi-class-100-9'+str(cfg.data['chain_len']),
                dir='/root/workspace/out/',
                tags=['synth-chains', 'chebnet', str(cfg.data['chain_len'])],
