...
from torch_geometric.nn import GCNConv, GINConv, GATConv, MessagePassing

from datasets.peptides_dataset import peptide_dataloaders
...
class Model(Module):
    def __init__(self,
        ...
        nonlin_module = ReLU()
        bias_module = GCNConv(hidden_channels, hidden_channels, normalize=False, cache=False, bias=True)
        self.igl = ImplicitLayer(nonlin_module, bias_module, phantom_grad=phantom_grad, beta_init=beta_init, gamma_init=gamma_init, tol=tol, sigmoid=True, max_iter=max_iter)
        pass
    ...
    def forward(self, data, x, edge_index, edge_weight, batch, u0=None):
        ...
        x = self.act(x)
        x = self.enc(x)
        x = self.igl(x, edge_index, edge_weight, u0=data.u0, bias_args={'edge_weight':edge_weight, 'edge_index':edge_index})
        x = self.norm(x)
        x = self.act(x)
...
def train(cfg, data, model, optimizer):
    model.train()
    optimizer.zero_grad()
    output, u0 = model(data, x=data.x, edge_index=data.edge_index, edge_weight=data.edge_weight, batch=data.batch, u0=data.u0)
    data.u0 = u0
...
def validate(cfg, data, model):
    model.eval()
    output, _ = model(data, x=data.x, edge_index=data.edge_index, edge_weight=data.edge_weight, batch=data.batch, u0=data.u0)
    loss = F.binary_cross_entropy_with_logits(output, data.y)
    return loss.item(), output
