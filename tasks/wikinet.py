...
import random
import torch
from torch.nn import Linear, Module, ModuleList, ReLU, Sequential, Tanh
...
from datasets.wikinet_dataset import wikinet_dataloaders
...
class Model(Module):
    def __init__(self,
        ...
        # self.dec = Sequential(Linear(hidden_channels, hidden_channels, bias=False), ReLU(), Linear(hidden_channels, out_channels, bias=False))
        ...
        self.igl_modules = ModuleList()
        for i in range(4):
            self.igl_modules.append(ImplicitLayer(nonlin_module, bias_module, phantom_grad=phantom_grad, beta_init=beta_init, gamma_init=gamma_init, tol=tol))
    ...
    def forward(self, x, edge_index, edge_weight, u0=None):
        ...
        x0 = x
        for igl in self.igl_modules:
            x =  self.act( igl(x, edge_index, edge_weight, u0=u0) + x0)
...
def load(cfg):
    ...
    dataset, data, data.train_mask, data.val_mask, data.test_mask = wikinet_dataloaders(
...
@hydra.main(version_base=None, config_path="/root/workspace/PR-inspired-aggregation/tasks/", config_name="wikinet")
def run_wikinet(cfg):
